...
<div class="container mx-auto pt-0 px-6 pb-6">
    ...
    <form action="<?php echo e(route('items.update', $item->id)); ?>" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-6">
        <?php echo csrf_field(); ?>
        <?php echo method_field('PUT'); ?>
        
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-[var(--table-data-color)]">Nombre</label>
            <input type="text" name="name" id="name" value="<?php echo e(old('name', $item->name)); ?>" 
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" 
                      class="mt-1 block w-full px-3 py-2 border border-[var(--tertiary-color)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] sm:text-sm"><?php echo e(old('description', $item->description)); ?></textarea>
        </div>
        
        <div class="mb-4">
            <label for="price" class="block text-sm font-medium text-[var(--table-data-color)]">Precio</label>
            <input type="number" step="0.01" name="price" id="price" value="<?php echo e(old('price', $item->price)); ?>" 
                   class="mt-1 block w-full px-3 py-2 border border-[var(--tertiary-color)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] sm:text-sm" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="category_id" id="category_id" 
                ...
                <?php $__currentLoopData = $categorias; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $categoria): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($categoria->id); ?>" <?php echo e(old('category_id', $item->category_id) == $categoria->id ? 'selected' : ''); ?>>
    ...
    <select name="branch_id" id="branch_id" 
        ...
        <?php $__currentLoopData = $branches; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $branch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <option value="<?php echo e($branch->id); ?>" 
                    <?php echo e(old('branch_id', $item->branch_id) == $branch->id ? 'selected' : ''); ?>>
        ...
        <div class="mb-6 p-4 border border-[var(--tertiary-color)] rounded-md bg-gray-50">
            ...
            <div class="mb-4 p-3 bg-white border border-[var(--tertiary-color)] rounded-md">
                ...
                <div class="flex items-center gap-4">
                    <?php if(filter_var($item->image, FILTER_VALIDATE_URL)): ?>
                        <img src="<?php echo e($item->image); ?>" alt="<?php echo e($item->name); ?>" class="h-24 w-24 object-cover rounded border">
                        <div class="text-sm text-gray-600">
                            <p class="font-medium">Tipo: URL externa</p>
                            <p class="text-xs text-gray-500 break-all"><?php echo e(Str::limit($item->image, 50)); ?></p>
                        </div>
                    <?php else: ?>
                        <img src="<?php echo e(asset('storage/' . $item->image)); ?>" alt="<?php echo e($item->name); ?>" class="h-24 w-24 object-cover rounded border">
                        <div class="text-sm text-gray-600">
                            <p class="font-medium">Tipo: Archivo local</p>
                            <p class="text-xs text-gray-500"><?php echo e(basename($item->image)); ?></p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-[var(--table-data-color)] mb-2">
                    <?php echo e($item->image ? 'Cambiar imagen (dejar vacío para mantener la actual)' : 'Agregar imagen'); ?>
        ...
        <div class="mb-6 p-4 border border-[var(--tertiary-color)] rounded-md bg-gray-50">
            ...
            <div class="mb-4">
                <div class="flex items-center">
                    <input type="checkbox" name="manage_inventory" id="manage_inventory" value="1" 
                           <?php echo e(old('manage_inventory', $item->manage_inventory) ? 'checked' : ''); ?>
            ...
            <div id="inventory-fields" class="space-y-4" style="display: <?php echo e(old('manage_inventory', $item->manage_inventory) ? 'block' : 'none'); ?>;">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="stock" id="stock" 
                               value="<?php echo e(old('stock', $item->stock ?? 0)); ?>" 
                               class="mt-1 block w-full px-3 py-2 border border-[var(--tertiary-color)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] sm:text-sm">
                        <p class="mt-1 text-xs text-gray-500">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="min_stock" id="min_stock" 
                               value="<?php echo e(old('min_stock', $item->min_stock ?? 5)); ?>" 
                               class="mt-1 block w-full px-3 py-2 border border-[var(--tertiary-color)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] sm:text-sm">
                        <p class="mt-1 text-xs text-gray-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="stock_type" id="stock_type" 
                                class="mt-1 block w-full px-3 py-2 border border-[var(--tertiary-color)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] sm:text-sm">
                            <option value="discrete" <?php echo e(old('stock_type', $item->stock_type) == 'discrete' ? 'selected' : ''); ?>>
                                Discreto (unidades)
                            </option>
                            <option value="continuous" <?php echo e(old('stock_type', $item->stock_type) == 'continuous' ? 'selected' : ''); ?>>
                    ...
                    <div>
                        ...
                        <input type="text" name="stock_unit" id="stock_unit" 
                               value="<?php echo e(old('stock_unit', $item->stock_unit ?? 'unidades')); ?>" 
                               class="mt-1 block w-full px-3 py-2 border border-[var(--tertiary-color)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] sm:text-sm">
                    </div>
                </div>
                
                <?php if($item->manage_inventory && $item->stock < $item->min_stock): ?>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="mt-1 text-sm text-yellow-700">
                                El stock actual (<?php echo e($item->stock); ?>) está por debajo del mínimo (<?php echo e($item->min_stock); ?>). 
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\HP\Desktop\laravel\repo\restaurant_app\resources\views/items/edit.blade.php ENDPATH**/ ?>