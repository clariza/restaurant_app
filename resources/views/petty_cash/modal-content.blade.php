            ...
            <div class="modal-content">
                ...
                <div class="expenses-section">
                    ...
                    <div class="expenses-container" id="expensesContainerClosure">
                        {{-- Solo gastos source='modal' --}}
                        @if(isset($existingExpenses) && $existingExpenses->count() > 0)
                            @foreach($existingExpenses as $expense)
                                <div class="expense-row expense-row--saved" data-expense-id="{{ $expense->id }}">
                        ...
                        <div class="section-container">
                            ...
                            <div class="form-grid">
                                <div class="input-group">
                                    ...
                                    {{--
                                        value           → todos los gastos (modal + form) de esta caja
                                        data-gastos-bd  → solo gastos modal (suma de filas --saved)
                                        data-gastos-form → gastos form (fijos, sin filas en el DOM)
                                        _refreshTotalGastos recalcula:
                                          total = data-gastos-form + suma(filas --saved actuales)
                                    --}}
                                    <input type="number"
                                           id="total-gastos-closure"
                                           class="form-control"
                                           value="{{ number_format($totalExpenses ?? 0, 2, '.', '') }}"
                                           data-gastos-bd="{{ number_format($totalExpensesModal, 2, '.', '') }}"
                                           data-gastos-form="{{ number_format($totalExpensesForm, 2, '.', '') }}"
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" id="ventas-qr-closure"
                                           class="form-control"
                                           value="{{ number_format($totalSalesQR ?? 0, 2, '.', '') }}"
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" id="ventas-tarjeta-closure"
                                           class="form-control"
                                           value="{{ number_format($totalSalesCard ?? 0, 2, '.', '') }}"
                                           step="0.01" min="0" autocomplete="off">
                                </div>
...
{{--
    ⚠️  NO hay <script> aquí a propósito.
    Los fragmentos inyectados vía innerHTML NO ejecutan sus <script>.
    Toda la lógica de cálculo de denominaciones vive en petty-cash-modal.js,
    que ya está cargado en el documento padre (app.blade.php).
--}}