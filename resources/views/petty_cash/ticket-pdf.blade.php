...
<table class="info-table">
    ...
    <tr>
        ...
        <td class="value">{{ \Carbon\Carbon::parse($pettyCash->closed_at)->format('d/m/Y H:i') }}</td>
    </tr>
    @endif
    <tr>
    ...
    <tr>
        ...
        <td class="value">{{ $pettyCash->sales()->count() }}</td>
    </tr>
</table>
...
<table class="cmp-table">
    ...
    <tbody>
        <tr>
            <td>Efectivo</td>
            <td class="r">{{ number_format($salesCashSystem, 2) }}</td>
            <td class="r">{{ number_format($salesCashBox, 2) }}</td>
            <td class="r"><strong>{{ $diffSign($diffCash) }}{{ number_format($diffCash, 2) }}</strong></td>
        </tr>
        <tr>
            <td>QR</td>
            <td class="r">{{ number_format($salesQRSystem, 2) }}</td>
            <td class="r">{{ number_format($salesQRBox, 2) }}</td>
            <td class="r"><strong>{{ $diffSign($diffQR) }}{{ number_format($diffQR, 2) }}</strong></td>
        </tr>
        <tr>
            <td>Tarjeta</td>
            <td class="r">{{ number_format($salesCardSystem, 2) }}</td>
            <td class="r">{{ number_format($salesCardBox, 2) }}</td>
            <td class="r"><strong>{{ $diffSign($diffCard) }}{{ number_format($diffCard, 2) }}</strong></td>
        </tr>
        <tr class="total">
            <td>TOTAL</td>
            <td class="r">{{ number_format($totalSalesSystem, 2) }}</td>
            <td class="r">{{ number_format($totalSalesBox, 2) }}</td>
            <td class="r">{{ $diffSign($diffTotal) }}{{ number_format($diffTotal, 2) }}</td>
        </tr>
    </tbody>
</table>
...
<div class="summary-box">
    <table class="summary-table">
        <tr>
            ...
            <td style="text-align:right">Bs. {{ number_format($totalSalesBox, 2) }}</td>
        </tr>
        <tr>
            <td>Total Gastos:</td>
            <td style="text-align:right">Bs. {{ number_format($totalExpenses, 2) }}</td>
        </tr>
        <tr>
            <td>SALDO FINAL:</td>
            <td style="text-align:right">Bs. {{ number_format($totalSalesBox - $totalExpenses, 2) }}</td>
        </tr>
    </table>
</div>
        ...
        <tr>
            <td class="expense-name" style="padding:1.2mm 0; border-bottom:1px dotted #bbb;">
                ...
                @if(!empty($expense->description))
                    <br><span class="expense-desc">{{ $expense->description }}</span>
                @endif
            </td>
            <td style="text-align:right; padding:1.2mm 0; border-bottom:1px dotted #bbb; font-size:8.5px;">
                <strong>Bs. {{ number_format($expense->amount, 2) }}</strong>
            </td>
        </tr>
        @endforeach
