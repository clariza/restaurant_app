    ...
    <form action="{{ route('suppliers.store') }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="mb-4">
            ...
            <textarea name="address" id="address" rows="2"
                      ...
                      placeholder="Ingrese la dirección completa del proveedor" required>{{ old('address') }}</textarea>
            @error('address')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="notes" id="notes" rows="4"
                      class="mt-1 block w-full px-3 py-2 border border-[var(--tertiary-color)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] sm:text-sm resize-vertical" 
                      placeholder="Agregue cualquier información adicional sobre el proveedor (términos de pago, observaciones, etc.)">{{ old('notes') }}</textarea>
            @error('notes')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
